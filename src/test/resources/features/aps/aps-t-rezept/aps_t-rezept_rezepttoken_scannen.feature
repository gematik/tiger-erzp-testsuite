...
#Test für APS
#Testfall prüft das Abrufen eines T-Rezeptes mit gescanntem Token. Zu erfüllende Vorbedingungen:
#   1) AuthN-Token anfordern für VPS,
#   2) T-Rezept erzeugen,
#   3) T-Rezept einstellen,
#   4) AuthN-Token anfordern für APS

@APS_T-REZEPT_REZEPTTOKEN_SCANNEN
@APS_T-REZEPT
Funktion: eRp abgebend - GF T-Rezept abrufen
  ...
  Szenario: E-Rezept abrufen und Signatur prüfen
    Gegeben sei TGR pausiere Testausführung mit Nachricht "Bitte rufen Sie als Apotheker das E-T-Rezept mit TaskId: ${erp.task_id} und AccessCode: ${erp.task_access_code} ab und prüfen Sie anschließend die Signatur des empfangenen eRezepts."
