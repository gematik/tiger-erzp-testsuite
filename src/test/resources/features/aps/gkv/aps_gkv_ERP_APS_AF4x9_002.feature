...
#Test für APS
# Testfall prüft das erneute Abrufen einer Quittung für ein E-Rezeptes. Zu erfüllende Vorbedingungen:
...
@APS_AF4x9_002
@APS
Funktion: eRp abgebend - ERP_APS_AF4x9_002 - GF Gelesene Nachricht durch Abgebenden löschen
  ...
  """
    ...
    Dann TGR prüfe aktuelle Antwort stimmt im Knoten "$.responseCode" überein mit "201"
    Und TGR prüfe aktuelle Antwort stimmt im Knoten "$.body" nicht überein mit "^Error:.*"
    Und TGR speichere Wert des Knotens "$.body.id" der aktuellen Antwort in der Variable "erp.patient_communication_id"

  @APS
  Szenario: Vorbedingung: Apotheke liest Zuweisung
    Gegeben sei TGR pausiere Testausführung mit Nachricht "Bitte lesen Sie die Zuweisung des Patienten mit Id: ${erp.patient_communication_id}"
    Und TGR finde die letzte Anfrage mit Pfad ".*" und Knoten "$.body.message.path.basicPath" der mit "/Communication/${erp.patient_communication_id}" übereinstimmt
    Dann TGR prüfe aktuelle Antwort stimmt im Knoten "$.body.message.responseCode" überein mit "200"
    Und TGR prüfe aktuelle Antwort stimmt im Knoten "$.body" nicht überein mit "^Error:.*"

  @APS
  Szenario: Vorbedingung: Apotheke sendet Antwort
    Gegeben sei TGR pausiere Testausführung mit Nachricht "Bitte senden Sie eine Antwort"
    ...
    Und TGR prüfe aktuelle Antwort stimmt im Knoten "$.body" nicht überein mit "^Error:.*"
    Und TGR speichere Wert des Knotens "$.body..id.value" der aktuellen Antwort in der Variable "erp.communication_id"

  @APS
  Szenario: Vorbedingung: Patient liest Nachricht
    Und TGR setze folgende default headers:
  """
    Content-Type  = application/fhir+json; charset=UTF-8
    Accept        = application/fhir+json
  ...
  """
    ...
    Wenn TGR sende eine leere GET Anfrage an "${data.address_fachdienst_fdv}/Communication/${erp.communication_id}"
    Und TGR finde die letzte Anfrage mit dem Pfad "/Communication/${erp.communication_id}"
    Dann TGR prüfe aktuelle Antwort stimmt im Knoten "$.responseCode" überein mit "200"
    Und TGR prüfe aktuelle Antwort stimmt im Knoten "$.body" nicht überein mit "^Error:.*"

  @APS
  Szenario: Gelesene Nachricht durch Abgebenden löschen
    Gegeben sei TGR pausiere Testausführung mit Nachricht "Bitte löschen Sie ihre eingestellte Nachricht."
    Und TGR finde die letzte Anfrage mit Pfad ".*" und Knoten "$.body.message.path.basicPath" der mit "/Communication/${erp.communication_id}" übereinstimmt
