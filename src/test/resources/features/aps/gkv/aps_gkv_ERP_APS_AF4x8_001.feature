...
@APS_AF4x8_001
@APS
Funktion: eRp abgebend - ERP_APS_AF4x8_001 - GF E-Rezept Quittung erneut abrufen
  ...
  Szenario: Vorbedingung: Als Apotheker die E-Rezept-Abgabe vollziehen
    Gegeben sei TGR pausiere Testausführung mit Nachricht "Bitte vollziehen Sie als Apotheker die E-Rezept-Abgabe mit TaskId: ${erp.task_id} und TaskSecret: ${erp.task_secret}"
  ...
  @APS
  Szenario: Quittung erneut abrufen
    Gegeben sei TGR pausiere Testausführung mit Nachricht "Bitte rufen Sie die Quittung für ein E-Rezept erneut ab mit TaskId: ${erp.task_id} und TaskSecret: ${erp.task_secret}"
    Und TGR finde die letzte Anfrage mit Pfad ".*" und Knoten "$.body.message.path.basicPath" der mit "/Task/${erp.task_id}" übereinstimmt
    Dann TGR prüfe aktuelle Antwort stimmt im Knoten "$.body.message.responseCode" überein mit "200"
